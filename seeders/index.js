...
async function seedUser () {
  try {
    ...
    userToBeInserted = {
      'password':'N28xUJOmSDBUG6q',
      'isDeleted':false,
      'username':'Kyle.Kunze80',
      'email':'Bella_Corwin@gmail.com',
      'isActive':true,
      'userType':authConstant.USER_TYPES.User
    };
    userToBeInserted.password = await  bcrypt.hash(userToBeInserted.password, 8);
    let user = await dbService.updateOne(User, { 'username':'Kyle.Kunze80' }, userToBeInserted,  { upsert: true });
    userToBeInserted = {
      'password':'kda_oJ4dzTY35At',
      'isDeleted':false,
      'username':'Tatyana.Runolfsson',
      'email':'Alvera.Upton@yahoo.com',
      'isActive':true,
      'userType':authConstant.USER_TYPES.Admin
    };
    userToBeInserted.password = await  bcrypt.hash(userToBeInserted.password, 8);
    let admin = await dbService.updateOne(User, { 'username':'Tatyana.Runolfsson' }, userToBeInserted,  { upsert: true });
